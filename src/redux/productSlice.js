...
const getErr = (e) => {
  ...
  if (isLikelyHtmlGatewayBody(d)) return apiUnreachableMessage();
  if (typeof d === 'string' && d.length > 200 && d.includes('<')) return apiUnreachableMessage();
  return d?.msg || d?.message || e.message || 'Failed';
};
